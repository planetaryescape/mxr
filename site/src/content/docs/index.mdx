...
If you want a specific tag instead of `main`:
...
mxr syncs your email to a local SQLite database and indexes it with Tantivy — the same engine behind [Quickwit](https://quickwit.io). Opening a message is a local read, not a network call. Search uses BM25 ranking with field-level boosts, plus optional local semantic retrieval when you enable it. Works offline. Your data lives on your machine. No spinners. No loading states.
