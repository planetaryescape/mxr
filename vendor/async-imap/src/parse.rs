...
pub(crate) async fn parse_list_status<T: Stream<Item = io::Result<ResponseData>> + Unpin>(
    stream: &mut T,
    unsolicited: channel::Sender<UnsolicitedResponse>,
    command_tag: RequestId,
    ...
    let mut statuses: HashMap<String, Mailbox> = HashMap::new();
    ...
    while let Some(resp) = stream.next().await {
        ...
        match resp.parsed() {
            ...
            Response::MailboxData(MailboxDatum::List { name, .. }) => {
                names.insert(name.to_string(), Name::from_mailbox_data(resp));
            }
            Response::MailboxData(MailboxDatum::Status { mailbox, status }) => {
                let entry = statuses.entry(mailbox.to_string()).or_default();
                for attribute in status {
                    match attribute {
                        StatusAttribute::HighestModSeq(highest_modseq) => {
                            entry.highest_modseq = Some(*highest_modseq)
                        }
                        StatusAttribute::Messages(exists) => entry.exists = *exists,
                        StatusAttribute::Recent(recent) => entry.recent = *recent,
                        StatusAttribute::UidNext(uid_next) => entry.uid_next = Some(*uid_next),
                        StatusAttribute::UidValidity(uid_validity) => {
                            entry.uid_validity = Some(*uid_validity)
                        }
                        StatusAttribute::Unseen(unseen) => entry.unseen = Some(*unseen),
                        _ => {}
                    }
                }
            }
    ...
    for (mailbox, name) in names {
        result.push(ListStatus {
            name,
            mailbox: statuses.remove(&mailbox).unwrap_or_default(),
        });
    }
    Ok(result)
}

pub(crate) async fn parse_qresync<T: Stream<Item = io::Result<ResponseData>> + Unpin>(
    stream: &mut T,
    unsolicited: channel::Sender<UnsolicitedResponse>,
    command_tag: RequestId,
    ...
    while let Some(resp) = stream.next().await {
        ...
        match resp.parsed() {
            ...
            Response::Data {
                ...
                use imap_proto::{ResponseCode, Status};
                match status {
                    Status::Ok => match code {
                        Some(ResponseCode::UidValidity(uid)) => mailbox.uid_validity = Some(*uid),
                        Some(ResponseCode::UidNext(uid_next)) => mailbox.uid_next = Some(*uid_next),
            ...
            Response::MailboxData(m) => match m {
                MailboxDatum::Exists(e) => mailbox.exists = *e,
                MailboxDatum::Recent(r) => mailbox.recent = *r,
                MailboxDatum::Flags(flags) => {
                    mailbox
                        .flags
                        .extend(flags.iter().map(|s| (*s).to_string()).map(Flag::from));
            ...
            Response::Fetch(..) => fetches.push(Fetch::new(resp)),
            _ => handle_unilateral(resp, unsolicited.clone()),
        }
    }
