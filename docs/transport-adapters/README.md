...
**Status: all phases 1–6 complete** (2026-07-19). The transport system now has everything the provider system has: trait, capabilities, conformance, skeleton, blueprint, and a decision-log trail. The single reference is `docs/blueprint/20-transports.md`.
...
- 2026-07-19 — Phase 5 implemented (5a TCP+token, 5b stdio server, 5c `cmd://` connector; 5d deferred). The one additive protocol change landed: `Request::Authenticate { token }` → `ResponseData::Authenticated`, **no `IPC_PROTOCOL_VERSION` bump** (additive-only; old clients never emit it, and the build-id handshake already forces a daemon restart on any upgrade — see 05 spec). New transports: `TcpServerTransport` (loopback-only bind, refuses non-loopback; `PeerAuth::TokenRequired`) + `TcpConnector` (auto-authenticates when it carries a token); a `cmd://` `CmdConnector` (spawn-and-pipe, kill-on-drop, stderr passthrough); and `mxr daemon --stdio` (one connection over stdin/stdout, `PeerAuth::LocalProcess`, stdout frame-clean). The serve core gained a **connection-scoped auth gate**: `TokenRequired` peers get `IpcErrorKind::Auth` on every request (and no events) until a successful `Authenticate`; UDS/memory/stdio start trusted and are unchanged. Token store: a **dedicated** IPC secret (`MXR_DAEMON_TOKEN` env > `<config_dir>/daemon-token`, 0600 atomic create), distinct from the HTTP bridge token — reusing the bridge token would leak, since its `/api/v1/auth/local-token` endpoint hands it to any loopback caller. The gate comparison is constant-time; the `TcpConnector` refuses non-loopback targets so the token is never sent in plaintext to a remote host. Config: opt-in `[transports.tcp]` (disabled by default). Clients: the CLI builds its connector from `MXR_DAEMON_ADDR` (`unix://`/`tcp://`/`cmd://`); TUI/web/MCP route socket resolution through the shared resolver and **reject** `tcp://`/`cmd://` with a clear message (no silent fallback). Corpus: scenarios 1–13 now run over a fifth harness (real TCP+token, post-`Authenticate`) = 65 matrixed tests, plus a bespoke scenario-14 auth matrix (pre-auth reject / bad token reject / good token unlock) and the no-auth pins for the four implicit-trust transports. Decision gates resolved: Q2 (no in-daemon remote — loopback only, refuse non-loopback), Q5 (in-process bridge — **deferred**, see below). Deferred: 5d (daemon-hosted bridge in-process transport) — the win requires rethreading `mxr-web`'s ~50 `socket_path` call sites onto a `Connector`; landing seam is the two connect points at `crates/web/src/lib.rs` (`ipc_request_with_id`, `bridge_events`). The standalone `mxr web` and daemon-hosted bridge both still dial the UDS socket; behavior is unchanged.
- 2026-07-19 — Phase 6 implemented (community adapter kit & docs — the final phase). Conformance packaging **split by contract** (D057): `mxr-transport` gains a `conformance` feature exporting `run_transport_conformance` / `run_token_auth_conformance` (the transport contract — bind/accept/`stop_accepting`/`cleanup`, cancel-safety, bidirectional stream, `PeerInfo`↔capability coherence; protocol-free, no daemon), while the daemon keeps the protocol corpus (`ipc_conformance.rs`) it already runs over every transport. The spec's original 6a (export a fake-provider serve core from `mxr-test-support`) was rejected: it would pull the whole daemon into an adapter's dev-deps, breaking the "depends only on `mxr-transport`, without reading daemon source" exit criteria. Proven end-to-end by an out-of-tree scratch crate that implements its own transport and passes the suite depending only on `mxr-transport`. New `examples/transport-skeleton/` (standalone `[workspace]`, `publish = false`, all-stub `ExampleServerTransport`/`ExampleListener`/`ExampleConnector`, `TransportCapabilities::default()`, commented conformance `#[tokio::test]`), excluded from the root workspace and built in its own CI job like the adapter skeleton. New blueprint chapter `docs/blueprint/20-transports.md` (architecture, trait reference, the per-transport security table, the adapter-kit checklist, the out-of-process `dial-stdio`/`cmd://` recommended path). Decision log: D055 (byte-stream abstraction, Podman/tarpc), D056 (auth evidence in `PeerInfo`, Tailscale), D057 (conformance split); HTTP-gateway and remote-out-of-process / no-protocol-bump already logged at D052/D054. Backlog (named pipes / sd_listen_fds / vsock / WS-binary / TLS remote) and two known follow-ups recorded in 20-transports §6: the deferred 5d in-process bridge, and the phase-4 UDS `stop_accepting` test flake (a single-attempt post-stop connect assertion that races the kernel backlog drain under parallel load — a test timing artifact, not a transport bug). No new transports, no capability changes, no behavior change to shipped transports.
