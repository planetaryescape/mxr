...
0. Preflight. `git fetch origin --tags`, then compare `.release-please-manifest.json` on `origin/main` with `git tag --sort=-v:refname | head -1`. If they disagree, land a PR that sets the manifest to the latest tag's version first. Also check `gh pr list --head release-please--branches--main`: an open release PR whose version is already tagged is stale; close it and let release-please regenerate it.
1. Commit release-ready changes on a branch with conventional commits. Run the scoped local gates above, nothing wider. A release only cuts when, since the last tag, there is at least one `feat:` or `fix:` commit (pre-1.0 both bump the patch; `chore:`/`docs:`/`ci:` do not) and at least one artifact-affecting change (`crates/`, `apps/web/`, real Cargo dependency changes, release workflows/scripts; see `scripts/release_change_scope.sh`). Otherwise `release-please.yml` skips its job and no release PR appears.
