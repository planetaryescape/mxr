...
pub trait MailSyncProvider: Send + Sync {
    ...
    /// Apply a mutation against the provider. Every message mutation
    /// (labels, trash, read, starred, keywords) arrives through this one
    /// entry point as a `Mutation` variant.
    ///
    /// Callers supply a stable `mutation_id` (UUIDv7) so the daemon's
    /// `mutation_dedup_log` (24h window) can skip a duplicate apply on
    /// retry. Adapters may ignore it — the authoritative dedup happens in
    /// the daemon's store.
...
/// Capabilities are grouped by concern rather than held flat.
...
pub struct SyncCaps {
    pub delta: bool,            // Incremental cursor-driven sync (Gmail: yes via history.list, IMAP: yes/partial)
    pub native_threading: bool, // Native provider thread ids (Gmail: yes, IMAP: no → JWZ fallback)
}

pub struct MutateCaps {
    pub labels: bool,           // Stable multi-assign labels (Gmail: yes, IMAP folders: no)
    pub batch_operations: bool, // Batch API calls (Gmail: yes, IMAP: UID STORE ranges)
    pub custom_keywords: bool,  // Round-trips arbitrary `$Foo` keywords (IMAP/JMAP: yes, Gmail: no)
}

pub struct SearchCaps {
    pub server_side: bool,      // Provider can search remotely; app may still choose local search
}

pub struct PushCaps {
    pub streaming: bool,        // Long-lived provider-side change channel (Gmail Pub/Sub deferred)
}
```

`mutate.labels == false` does not mean "no folders." It means callers must not assume Gmail-style stable multi-assign label semantics. Folder-backed providers may map the same `Mutation::ModifyLabels` request to move or copy behavior instead.

`search.server_side` is provider truth, not a promise that the app always routes search there.

`Mutation::SetKeywords` must be rejected with `MxrError::Provider` when `mutate.custom_keywords` is false, rather than silently dropped.
...
pub trait MailSendProvider: Send + Sync {
    ...
    /// Send a calendar RSVP. Default: not supported.
    ...
    /// Save a draft to the mail server if supported. Default: `Ok(None)`.
