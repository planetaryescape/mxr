...
impl<T: Read + Write + Unpin + fmt::Debug + Send> Session<T> {
    ...
    /// Enable server capabilities for this authenticated session.
    pub async fn enable<S: AsRef<str>>(&mut self, caps: &[S]) -> Result<Capabilities> {
        let command = format!(
            "ENABLE {}",
            caps.iter()
                .map(|cap| cap.as_ref())
    ...
    /// Return namespace information as defined by RFC 2342.
    pub async fn namespace(&mut self) -> Result<Namespace> {
        let responses = self.collect_responses("NAMESPACE").await?;
        let mut namespace = Namespace::default();
        for response in &responses {
            if let Some(raw) = response.raw_str() {
                if raw.starts_with("* NAMESPACE ") {
                    namespace = parse_namespace_response(raw)?;
                }
            }
        }
        Ok(namespace)
    ...
    /// LIST with STATUS return data in one roundtrip.
    pub async fn list_status<S1: AsRef<str>, S2: AsRef<str>>(
        &mut self,
        reference_name: S1,
        mailbox_pattern: S2,
    ) -> Result<Vec<ListStatus>> {
        let command = format!(
            "LIST {} {} RETURN (STATUS (MESSAGES UNSEEN UIDNEXT UIDVALIDITY HIGHESTMODSEQ))",
            quote!(reference_name.as_ref()),
            quote!(mailbox_pattern.as_ref())
        );
        let id = self.run_command(command).await?;
        parse_list_status(
            &mut self.conn.stream,
    ...
    /// SELECT a mailbox using QRESYNC parameters.
    pub async fn select_qresync<S1: AsRef<str>, S2: AsRef<str>>(
        &mut self,
        mailbox_name: S1,
        qresync_args: S2,
    ) -> Result<QresyncResponse> {
        let command = format!(
            "SELECT {} (QRESYNC ({}))",
            validate_str(mailbox_name.as_ref())?,
            qresync_args.as_ref()
    ...
    async fn collect_responses<S: AsRef<str>>(&mut self, command: S) -> Result<Vec<ResponseData>> {
        let id = self.run_command(command).await?;
        let mut responses = Vec::new();

        while let Some(response) = self.read_response().await {
            ...
            match response.parsed() {
                Response::Done {
                    ...
                    self.conn
                        .check_status_ok(status, code.as_ref(), information.as_deref())?;
                    break;
                }
...
fn parse_namespace_response(raw: &str) -> Result<Namespace> {
    let payload = raw
        .trim()
        .strip_prefix("* NAMESPACE ")
        .ok_or_else(|| Error::Parse(ParseError::Invalid(raw.as_bytes().to_vec())))?;
    let sections = split_namespace_sections(payload)?;
    Ok(Namespace {
        personal: parse_namespace_group(sections.first().copied().unwrap_or("NIL")),
        other_users: parse_namespace_group(sections.get(1).copied().unwrap_or("NIL")),
        shared: parse_namespace_group(sections.get(2).copied().unwrap_or("NIL")),
    })
}

fn split_namespace_sections(payload: &str) -> Result<Vec<&str>> {
    ...
    for (idx, ch) in payload.char_indices() {
        match ch {
            '"' if idx == 0 || bytes[idx.saturating_sub(1)] != b'\\' => in_quotes = !in_quotes,
            '(' if !in_quotes => depth += 1,
            ')' if !in_quotes => depth -= 1,
            ' ' if !in_quotes && depth == 0 => {
                sections.push(payload[start..idx].trim());
                start = idx + 1;
            }
            _ => {}
        }
    ...
    sections.push(payload[start..].trim());
    if sections.len() == 3 {
        Ok(sections)
    } else {
...
fn parse_namespace_group(section: &str) -> Vec<NamespaceEntry> {
    if section.eq_ignore_ascii_case("NIL") {
        return Vec::new();
    }
