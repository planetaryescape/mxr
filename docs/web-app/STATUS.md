...
| 3 | Compose | usable baseline | Working compose page, session restore, autosave, send confirm, CodeMirror+vim default, lazy Tiptap alternate, drag/drop attachment upload, snippets expansion. Remaining: contact autocomplete, outbound undo pill. |
| 4 | Search | usable baseline | Top-bar search, debounced local dropdown, full `/search` page, lexical/semantic/hybrid mode values aligned to bridge, token chips, syntax help, save-search dialog. Remaining: contact buckets, saved-search color/pin/update UX. |
| 5 | Command palette | usable baseline | Global `Cmd-K`, `/`, `:`, `?`, navigation/actions/mail/settings/lenses groups, analytics/account/rules/screener routes, recent-mail fetch, keyboard help. Remaining: explicit tabbed scopes and more mutation/context action coverage. |
| 6 | Analytics | usable baseline | Storage, stale, contacts, response-time, subscriptions, and wrapped dashboard routes backed by verified bridge endpoints. Remaining: story mode, right-rail drilldowns, share-as-image. |
| 7 | Rules | usable baseline | Rules list, form editor, save/delete, enable toggle, history, always-visible dry-run preview, apply-preview-now for common mail actions, and undo toast for mutation-backed runs. Remaining: typed builder rows and apply-now coverage for non-mail actions. |
| 8 | Accounts + onboarding | usable baseline | Accounts list/detail, default/disable/remove/test, aliases, primary alias action, re-auth session controls, first-run redirect, Gmail/Outlook auth sessions, IMAP+SMTP wizard, initial sync page. Remaining: richer sync settings. |
