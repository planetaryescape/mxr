...
- **Address resolution: one source for `unix://`.** For a `unix://` address the daemon's bind, autostart, the liveness/stale probe, doctor's reachability, and the request path all resolve through `TransportAddr::resolve` (precedence `MXR_DAEMON_ADDR` > `MXR_SOCKET_PATH` > per-instance default), so start / probe / request never disagree. `tcp://` and `cmd://` are **request-path overrides only**: the CLI dials the configured connector for requests, but autostart and the stale-socket probe are skipped and continue to operate on the default UDS instance (a TCP/`cmd` daemon manages its own lifecycle — you start it yourself, or the `cmd://` bridge does). The standalone `mxr-tui` / `mxr-web` / `mxr-mcp` clients resolve through the shared `TransportAddr::resolve_unix_socket` — `unix://` only, with `tcp://`/`cmd://` rejected with a clear message rather than silently ignored. See `docs/blueprint/20-transports.md` for the full transport reference.
